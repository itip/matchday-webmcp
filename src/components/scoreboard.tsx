  ...
  return <div className="site-shell">
    ...
    <main id="top">
      ...
      <div className="content-grid" id="fixtures"><div className="results-column">
        <div className="results-toolbar"><div><p>{selectedTeam ? 'TEAM VIEW' : followingOnly ? 'FOLLOWING' : 'MATCH CENTRE'}</p><h2>{heading}</h2></div><span>{filteredFixtures.length} matches</span>{selectedTeam && <div className="team-view-actions"><button aria-pressed={followedTeams.includes(selectedTeam)} onClick={() => toggleFollowTeam(selectedTeam)} type="button">{followedTeams.includes(selectedTeam) ? 'Following team ✓' : 'Follow team ＋'}</button><button type="button" onClick={() => void navigate({ search: (previous) => ({ ...previous, team: undefined }) })}>Clear team ×</button></div>}</div>
