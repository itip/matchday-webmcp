...
export function Scoreboard({ search }: { search: ScoresSearch }) {
  ...
  useEffect(() => {
    ...
    void document.modelContext.registerTool({
      ...
      execute: async () => ({ countries: countryOptions, competitions: competitionOptions, selected: { countries: selectedCountries, competitions: selectedCompetitions, status: search.status, date: search.date } }),
    }, { signal: controller.signal });
    void document.modelContext.registerTool({
      ...
      inputSchema: { type: 'object', properties: { countries: { type: 'array', items: { type: 'string' }, description: 'Country names from get_match_filter_options. Use an empty array for all countries.' }, competitions: { type: 'array', items: { type: 'string' }, description: 'Competition names from get_match_filter_options. Use an empty array for all competitions.' }, date: { type: 'string', description: 'Optional fixture date in YYYY-MM-DD format.' }, status: { type: 'string', enum: ['all', 'live', 'upcoming'], description: 'Optional match status filter.' } }, required: ['countries', 'competitions'], additionalProperties: false },
      execute: async ({ countries, competitions, date, status }: { countries: string[]; competitions: string[]; date?: string; status?: 'all' | 'live' | 'upcoming' }) => { const validCountries = canonicalSelections(countries, countryOptions); const validCompetitions = canonicalSelections(competitions, competitionOptions); await navigate({ search: (previous) => ({ ...previous, countries: encodeSelection(validCountries), competitions: encodeSelection(validCompetitions), date: date ?? previous.date, status: status ?? previous.status }) }); setAnnouncement(`Filters updated: ${validCountries.length || 'all'} countries and ${validCompetitions.length || 'all'} competitions.`); return { changed: true, countries: validCountries, competitions: validCompetitions, date: date ?? search.date, status: status ?? search.status }; },
    }, { signal: controller.signal });
    return () => controller.abort();
  }, [navigate, search.date, search.status, search.countries, search.competitions]);
  ...
  return <div className="site-shell">
    ...
    <main id="top">
      <section className="score-hero" id="scores"><div><p className="eyebrow"><span className="live-dot" /> {dateFixtures.length} matches · {competitionCount} competitions loaded</p><h1>Scores & fixtures</h1><p className="hero-copy">Every match. Every level. One place.</p></div><form className="team-search" onSubmit={submitSearch}><label htmlFor="team-search">Find a team or competition</label><div><input id="team-search" type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Try AFC Telford United" /><button type="submit">Search</button></div></form></section>
      <section className="date-strip" aria-label="Choose a date">{dates.map(([day, date, iso]) => <button className={!selectedTeam && search.date === iso ? 'selected' : ''} key={iso} onClick={() => void navigate({ search: (previous) => ({ ...previous, date: iso, team: undefined }) })} type="button"><span>{day}</span><strong>{date}</strong></button>)}<label className="calendar-button"><span>CAL</span><strong>▦</strong><input aria-label="Choose any fixture date" type="date" value={search.date} onChange={(event) => void navigate({ search: (previous) => ({ ...previous, date: event.target.value, team: undefined }) })} /></label></section>
